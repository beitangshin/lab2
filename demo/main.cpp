...
int main(int argc, char*argv[]) {
	...
	char modeChar;
	//{ CUDA, VECTOR, OMP, PTHREAD, SEQ };

	std::cout << "Enter the mode: CUDA = 0, VECTOR = 1, OMP = 2, PTHREAD = 3, SEQ = 4\n";
	std::cin >> modeChar;
	...
	{ // This scope is for the purpose of removing false memory leak positives
		...
		if (timing_mode)
		{
			...
			Ped::IMPLEMENTATION implementation_to_test = mode;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
