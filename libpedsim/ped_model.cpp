...
Ped::Model::Model():internalThread{threadnums}
{}
...
// Tick for agents
void tickAgents(int start, int end, std::vector<Ped::Tagent*> agents)
{
...
void Ped::Model::tick()
{
    ...
    int threadnums =internalThread;
		int sizeofAgents =agents.size();
		int piecesOfAgents = (sizeofAgents / threadnums)+1;

		for (int i = 0; i < threadnums; i++) {
			int start = i * piecesOfAgents;
			int end = (i + 1) * piecesOfAgents;
			if (end >= sizeofAgents) {
				end = sizeofAgents;
			}

			threads.push_back(std::thread(tickAgents, start, end, agents));
		}
